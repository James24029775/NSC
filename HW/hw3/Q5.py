...
def main():
    packet_num_range = range(100, 1050, 50)
    record = [[[] for i in range(CASE_NUM)] for j in range(CASE_NUM)]

    for i in range(CASE_NUM):
        for packet_num in packet_num_range:
    ...
    plt.title("Infulence of Packet Num")
    plt.ylabel("Success rate")
    plt.xlabel("Packet Num")
    plt.legend()
    plt.savefig('Question/Q5_success.png')
    ...
    plt.xlabel("Packet Num")
    plt.legend()
    plt.savefig('Question/Q5_idle.png')
    ...
    plt.xlabel("Packet Num")
    plt.legend()
    plt.savefig('Question/Q5_collision.png')
